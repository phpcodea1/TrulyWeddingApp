...
class AllMenuViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    @IBAction func saveMenuAtion(_ sender: Any)
    {
        if (menUname.text?.count == 0)
            
        {
            Helper.helper.showAlertMessage(vc: self, titleStr: "Notification", messageStr: "Please enter menu name")
        }
        else
        {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
    {
         ...
         var dict = self.MenuArray.object(at: section) as! NSDictionary
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        let cell = Bundle.main.loadNibNamed("NewAllGuestCellTableViewCell", owner: self, options: nil)?.first as! NewAllGuestCellTableViewCell
         ...
         var dict = self.MenuArray.object(at: indexPath.section) as! NSDictionary
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView?
    {
        let headercell = Bundle.main.loadNibNamed("NewAllGuestHeaderCell", owner: self, options: nil)?.first as! NewAllGuestHeaderCell
         ...
         if let group_name = dict.value(forKey: "menu_name") as? String
         {
    ...
    @objc func EditClick(_ sender: UIButton)
    {
        ...
        if let list_name = dict.value(forKey: "menu_name") as? String
        {
            self.menUname.text = list_name
            
        }
        if let table_id = dict.value(forKey: "menu_id") as? String
        {
    ...
    {
        var title1 = ""
        if let title = (self.GuestListArray2.object(at: row) as! NSDictionary).value(forKey: "menu_name") as? String
        {
            title1 = title
        ...
        var id1 = ""
        if let id = (self.GuestListArray2.object(at: row) as! NSDictionary).value(forKey: "menu_id") as? String
        {
    ...
    @objc func checkBoxButtonClick(_ sender: UIButton)
    {
        ...
        var dict3 = self.MenuArray.object(at: selectedSection) as! NSDictionary
        if self.subArray.count>0
        {
            self.subArray.removeAllObjects()
        }
        ...
        var dict = self.MenuArray.object(at: self.selectedSection) as! NSDictionary
        var table_name1 = dict.value(forKey: "menu_name") as! String
        
        
        for i in 0..<self.MenuArray.count
        {
            var dict2 = self.MenuArray.object(at: i) as! NSDictionary
            
            if dict2.value(forKey: "menu_name") as! String == table_name1
            {
    ...
    func GetMenuListAPI()
    {
        ...
        print(params)
        WebService.shared.apiDataPostMethod(url:GETMENULISTDATA, parameters: params) { (response, error) in
            if error == nil
            {
    ...
    func EditMenuAPI()
    {
        ...
        let params:[String:Any] =
            [
                "email":userEmail,
                "menu_id": self.edit_menu_id,
                "menu_name":self.menUname.text!,
                "event_id":eventID,
                "event_type":eventType,
                ]
        ...
        print(params)
        WebService.shared.apiDataPostMethod(url:EDITMENU, parameters: params) { (response, error) in
            if error == nil
            {
                self.blurView.isHidden = true
    ...
    func DeleteMenuAPI()
    {
        ...
        print(params)
        WebService.shared.apiDataPostMethod(url:DELETEMENU, parameters: params) { (response, error) in
            if error == nil
            {
                self.blurView.isHidden = true
                self.GetMenuListAPI()
    ...
    func MoveMenuAPI()
    {
        ...
        let params:[String:Any] =
            [
                "email":userEmail,
                "menu_id": self.pickerSelectedCatId,
        ...
        print(params)
        WebService.shared.apiDataPostMethod(url:MOVEEMENU, parameters: params) { (response, error) in
            if error == nil
            {
                self.blurView.isHidden = true
                self.GetMenuListAPI()
