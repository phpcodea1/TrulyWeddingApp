...
class EditGuestViewController: UIViewController,UIGestureRecognizerDelegate,UITableViewDelegate,UITableViewDataSource,UIPickerViewDelegate,UIPickerViewDataSource{
...
//                adultSegmt.setEnabled(true, forSegmentAt: 1)
        ...
        if let age_level = GuestDetails.value(forKey: "attendence") as? String
        {
            ...
            if age_level.lowercased() == "Not Attending"
            {
                //                adultSegmt.setEnabled(true, forSegmentAt: 1)
            ...
            if error == nil
            {
                ...
                if let data = (response as! NSDictionary).value(forKey: "data") as? NSArray
                {
                    ...
                    for i in 0..<self.tableArray.count
                    {
                        ...
                        var id = dict.value(forKey: "id") as! String
                        if let table_id2 = self.GuestDetails.value(forKey: "table_id") as? String
                        {
                            if table_id2 == id
                            {
                                self.table_id = id
...
//        if (emailTxt.text?.count == 0) || (firstNameTxt.text?.count == 0) || (lastNameTxt.text?.count == 0) || (groupInfomation.text?.count == 0) || (phoneNumberTxt.text?.count == 0) || (mobileNameTxt.text?.count == 0) || (addressTxt.text?.count == 0) || (cityTownTxt.text?.count == 0) || (stateTxt.text?.count == 0) || (postalCodeTxt.text?.count == 0) || (notes.text?.count == 0) || (selectTableTitle.title(for: .normal) == "Select table") || (selectListTitle.title(for: .normal) == "Select list") || (selectMenuTitle.title(for: .normal) == "Select menu")
        ...
        {
          Helper.helper.showAlertMessage(vc: self, titleStr: "Notification", messageStr: "Please enter last name.")
        }
        else
        {
            if CheckInternet.Connection()
            ...
            if error == nil
            {
                ...
                if let data = (response as! NSDictionary).value(forKey: "data") as? NSArray
                {
                    ...
                    for i in 0..<self.menuArray.count
                    {
                        ...
                        var id = dict.value(forKey: "id") as! String
                        if let menu_id2 = self.GuestDetails.value(forKey: "menu_id") as? String
                        {
                            if menu_id2 == id
                            {
                                self.menu_id = id
    ...
    func EditGuestAPI()
    {
        ...
        print(params)
        WebService.shared.apiDataPostMethod(url:EditGuestURL, parameters: params) { (response, error) in
            if error == nil
            {
                ...
                if let resp = response as? NSDictionary
                {
                    ...
                    {
                       // Helper.helper.showAlertMessage(vc: self, titleStr: "Message!", messageStr:" Update  successfully!")
                        self.navigationController?.popViewController(animated: true)
                    }
                    else
                    {
                ...
                if let data = (response as! NSDictionary).value(forKey: "data") as? NSArray
                {
                    ...
                    if let data = (response as! NSDictionary).value(forKey: "data") as? NSArray
                    {
                        ...
                        for i in 0..<self.categoryList.count
                        {
                            ...
                            var id = dict.value(forKey: "id") as! String
                            if let group_id2 = self.GuestDetails.value(forKey: "group_id") as? String
                            {
                                if  group_id2 == id
                                {
