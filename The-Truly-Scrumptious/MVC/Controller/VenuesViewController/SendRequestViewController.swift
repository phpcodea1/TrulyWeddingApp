...
class SendRequestViewController: UIViewController {
    ...
    {
        if  (nameTxt.text?.count == 0) || (emailTxt.text?.count == 0||phonetxt.text?.count == 0) || (messgeTxt.text?.count == 0) || (guestNumberTxt.text?.count == 0) || (eventTxt.text?.count == 0)
        {
    ...
    func SendRequestAPI()
    {
        ...
        print(params)
        WebService.shared.apiDataPostMethod(url:VENUEREQUESTPRICING, parameters: params) { (response, error) in
            if error == nil
            {
